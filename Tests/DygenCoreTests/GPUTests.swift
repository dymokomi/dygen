...
final class GPUTests: XCTestCase {
    ...
    func testReadWritePNGRoundTrip() throws {
        ...
        let img = try XCTUnwrap(ReadOp().evaluate(inputs: [:], params: ["path": .string(ref.path)], ctx: gpu))
        let tex = try XCTUnwrap(img.texture)
        ...
        _ = try WriteOp().evaluate(inputs: ["in": img], params: ["path": .string(out.path)], ctx: gpu)
        XCTAssertTrue(FileManager.default.fileExists(atPath: out.path))

        // The written PNG re-reads at the same dimensions.
        let back = try XCTUnwrap(ReadOp().evaluate(inputs: [:], params: ["path": .string(out.path)], ctx: gpu)?.texture)
    ...
    func testExecutorCachesReadOutput() throws {
        ...
        let a = try ex.evaluate(read.id, graph: graph)?.texture
        let b = try ex.evaluate(read.id, graph: graph)?.texture
        XCTAssertNotNil(a)
        ...
        let c = try ex.evaluate(read.id, graph: graph)?.texture
