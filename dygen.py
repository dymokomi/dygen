...
stencil_masks = []
stencil_masks.append( DYImage(f"{root_location}/tex/tex1.png").scaled( m * msk_m).expanded(source_image.size()))
stencil_masks.append( DYImage(f"{root_location}/tex/tex2.png").scaled( m * msk_m ).expanded(source_image.size()))
...
for i, color in enumerate(tqdm(palette)): source_image.paint(rounded_masks[i].blur(12*2).clamp(10).multed(stencil_masks[random.randint(0,1)],threshold=False), color, amount=0.5, threshold=False, volume_diff=0.03)
source_image2.blend(source_image, 0.5)
source_image2 = source_image2.mult(1.05)
for i, color in enumerate(tqdm(palette)): source_image.comp(source_image2, rounded_masks[i].multed(stencil_masks[random.randint(0,1)]))
